...
class ProductController extends Controller
{
    ...
    public function manage_product_process(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            // Save the original image to a directory
            $image->storeAs('/public/media', $image_name);

            // Resize the image using Intervention Image package
            $resizedImage = Image::make(public_path('storage/media') . '/' . $image_name)
                ->fit(250, 300)
                ->save(public_path('storage/media') . '/' . 'resized_' . $image_name);

            // Update the model with the resized image name
            $model->image = 'resized_' . $image_name;
        }
